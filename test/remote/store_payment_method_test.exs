defmodule Remote.StorePaymentMethodTest do
  ...
  test "invalid credentials" do
    ...
    { :error, message } = Environment.store_payment_method(bogus_env, "some_gateway", "some_card_token")
    assert message =~ "Unable to authenticate"
  end

  test "non existent payment method" do
    { :error, reason } = Environment.store_payment_method(env(), create_test_gateway().token, "non_existent_card")
  ...
  test "successfully store payment method" do
    {:ok, store_trans} = Environment.store_payment_method(env(), create_test_gateway().token, create_test_card().token)
