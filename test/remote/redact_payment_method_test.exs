defmodule Remote.RedactPaymentMethodTest do
  ...
  test "invalid credentials" do
    ...
    { :error, message } = Spreedly.redact_payment_method(bogus_env, "some_card_token")
    assert message =~ "Unable to authenticate"
  end

  test "non existent" do
    { :error, reason } = Spreedly.redact_payment_method(env(), "non_existent_card")
    assert reason =~ "Unable to find the specified payment method."
  end

  test "successfully redact" do
    {:ok, add_trans } = Spreedly.add_credit_card(env(), card_deets(retained: true))
    assert add_trans.payment_method.storage_state == "retained"
    {:ok, redact_trans} = Spreedly.redact_payment_method(env(), add_trans.payment_method.token)
