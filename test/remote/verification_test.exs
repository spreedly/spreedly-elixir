defmodule Remote.VerificationTest do
  ...
  test "invalid credentials" do
    ...
    { :error, reason } = Spreedly.verify(bogus_env, "gateway_token", "payment_method_token")
    assert reason =~ "Unable to authenticate"
  end

  test "payment method not found" do
    { :error, reason } = Spreedly.verify(env(), create_test_gateway().token, "unknown_card")
    assert reason =~ "There is no payment method"
  end

  test "gateway not found" do
    { :error, reason } = Spreedly.verify(env(), "unknown_gateway", create_test_card().token)
    assert reason == "Unable to find the specified gateway."
  end

  test "bogus currency" do
    {:error, reason } = Spreedly.verify(env(), create_test_gateway().token, create_test_card().token, "HUH")
  ...
  test "successful verify with legitimate currency_code" do
    {:ok, trans } = Spreedly.verify(env(), create_test_gateway().token, create_test_card().token, "GBP")
    assert trans.succeeded == true
    assert trans.currency_code == "GBP"
  end

  test "successful verify with options" do
    {:ok, trans } = Spreedly.verify(env(), create_test_gateway().token, create_test_card().token, "GBP", order_id: "44", description: "Wow")
    assert trans.succeeded == true
  ...
  test "failed verify" do
    {:ok, trans } = Spreedly.verify(env(), create_test_gateway().token, create_declined_test_card().token)
