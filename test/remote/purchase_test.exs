defmodule Remote.PurchaseTest do
  ...
  test "invalid credentials" do
    ...
    { :error, reason } = Spreedly.purchase(bogus_env, "gateway_token", "payment_method_token", 100)
    assert reason =~ "Unable to authenticate"
  end

  test "payment method not found" do
    { :error, reason } = Spreedly.purchase(env(), create_test_gateway().token, "unknown_card", 100)
    assert reason =~ "There is no payment method"
  end

  test "gateway not found" do
    { :error, reason } = Spreedly.purchase(env(), "unknown_gateway", create_test_card().token, 100)
  ...
  test "successful purchase with options" do
    {:ok, trans } = Spreedly.purchase(env(), create_test_gateway().token, create_test_card().token, 100, "GBP", order_id: "44", description: "Wow")
    assert trans.succeeded == true
  ...
  test "failed purchase" do
    {:ok, trans } = Spreedly.purchase(env(), create_test_gateway().token, create_declined_test_card().token, 100)
